...
void IRsend::sendSamsung(unsigned long data, int nbits)
{
  ...
  byte custom1, custom2, data1, data_compliment;

  custom1 = (data & 0xFF000000) >> 24;
  custom2 = (data & 0x00FF0000) >> 16;
  data1 = (data & 0x0000FF00) >> 8;
  data_compliment = data & 0x000000FF;
...
/*
void IRsend::sendSapien(int data)
{
  enableIROut(39);

  mark(SAPIEN_HDR_MARK);  // normally high
  space(SAPIEN_HDR_SPACE);  // start low for 8 * 1/1200

  data &= 0x00FF;  // take one byte

  Serial.print("code = ");
  for (int i = 0; i < 8; i++) {
    if (data & 0x0080) {	    // MSB comes first
      mark(SAPIEN_ONE_MARK);
      Serial.print("1");
    } 
    else {
      mark(SAPIEN_ZERO_MARK);
      Serial.print("0");
    }
    data <<=1;
    space(SAPIEN_BIT_SPACE);	  
  }
  mark(SAPIEN_HDR_MARK); // again goes high
...
void IRsend::sendSapien(int data)
{
  ...
  mark(SAPIEN_HDR_MARK);  // start high  for 8 * 1/1200

  for (int i = 0; i < 8; i++) {
    if (data & 0x0080) {	    // MSB comes first
      space(SAPIEN_ONE_SPACE);
    } 
    else {
      space(SAPIEN_ZERO_SPACE);
    }
    data <<=1;
    mark(SAPIEN_BIT_MARK);	  
  }
  space(0);
